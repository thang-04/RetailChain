...
const StoreStaffPage = () => {
    ...
    useEffect(() => {
        const fetchStaff = async () => {
            try {
                const data = await storeService.getStoreStaff(id);
                setStaffList(data);
            } catch (error) {
        ...
        <div className="max-w-[1200px] mx-auto flex flex-col gap-6 font-display text-slate-900 dark:text-white antialiased">
            ...
            <div className="flex flex-col bg-white dark:bg-[#1a1d21] rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                ...
                <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex flex-col lg:flex-row gap-4 justify-between items-start lg:items-center">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
                        <div className="relative w-full sm:w-72">
                            ...
                            <input className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm text-slate-900 dark:text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all" placeholder="Search staff by name or email..." type="text" />
                        </div>
                        <div className="relative w-full sm:w-48">
                            ...
                            <select className="w-full pl-10 pr-8 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary appearance-none cursor-pointer transition-all">
                                <option disabled="" selected="" value="">Filter by Role</option>
                                <option value="manager">Store Manager</option>
                                <option value="associate">Sales Associate</option>
                                <option value="specialist">Inventory Specialist</option>
                            </select>
                    ...
                    <div className="flex items-center gap-2 overflow-x-auto max-w-full pb-1 lg:pb-0">
                        <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-slate-100 dark:bg-slate-700 text-slate-900 dark:text-white text-xs font-semibold whitespace-nowrap transition-colors hover:bg-slate-200 dark:hover:bg-slate-600">
                            <span className="material-symbols-outlined text-[16px]">check_circle</span>
                            All
                        </button>
                        <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 text-xs font-medium whitespace-nowrap transition-colors hover:border-slate-300 dark:hover:border-slate-600 hover:text-slate-900 dark:hover:text-slate-200">
                            Active
                        </button>
                        <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 text-xs font-medium whitespace-nowrap transition-colors hover:border-slate-300 dark:hover:border-slate-600 hover:text-slate-900 dark:hover:text-slate-200">
                            On Leave
                ...
                <div className="p-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                    <p className="text-slate-500 dark:text-slate-400 text-sm">Showing <span className="font-semibold text-slate-900 dark:text-white">1-5</span> of <span className="font-semibold text-slate-900 dark:text-white">24</span> staff</p>
