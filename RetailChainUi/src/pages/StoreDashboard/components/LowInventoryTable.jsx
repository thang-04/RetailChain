    ...
    <Card className="bg-surface-light dark:bg-surface-dark border border-border-light dark:border-border-dark shadow-sm overflow-hidden flex-1">
      <CardHeader className="flex flex-row items-center justify-between py-4 px-6 border-b border-border-light dark:border-border-dark space-y-0">
        <div className="flex items-center gap-2">
          <CardTitle className="text-lg font-bold text-slate-900 dark:text-white">Sản phẩm tồn kho thấp</CardTitle>
        ...
        <Table>
          <TableHeader className="bg-slate-50 dark:bg-slate-800/50">
            <TableRow className="hover:bg-transparent border-b border-border-light dark:border-border-dark">
              <TableHead className="px-4 py-3 text-xs uppercase text-slate-500 font-semibold tracking-wider">Tên sản phẩm</TableHead>
              <TableHead className="px-4 py-3 text-xs uppercase text-slate-500 font-semibold tracking-wider text-right">Tồn kho</TableHead>
            ...
            {lowStockItems.map((item, index) => (
              <TableRow key={index} className="group hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors border-none">
                ...
                <TableCell className="px-4 py-3 text-right">
                  ...
                  <span className="text-xs text-slate-400 block">còn lại</span>
                </TableCell>
                <TableCell className="px-4 py-3 text-right">
                  {item.status === 'Ordered' ? (
                     <span className="text-xs text-slate-400 italic">{getStatusLabel(item.status)}</span>
                  ) : (
                    <Button variant="outline" size="sm" className="h-7 text-xs font-medium text-primary hover:text-primary-dark border-primary/30 hover:bg-primary/5">Đặt hàng lại</Button>
                  )}
                </TableCell>
              </TableRow>
            ))}
